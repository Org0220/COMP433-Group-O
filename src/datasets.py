    ...
    def __getitem__(self, idx):
        ...
        if label is not None:  # Supervised case
            if self.transform:
                ...
                print(f"Setting seed for supervised augmentation: {torch.initial_seed() + idx} (idx={idx})")
        ...
        if self.transform:
            ...
            print(f"Generated seeds for BYOL augmentation: seed1={seed1}, seed2={seed2} (epoch={self.epoch}, idx={idx})")
            
            # Create first view
            torch.manual_seed(seed1)
            view1 = self.transform(image)
            print(f"Transform applied to view1 with seed={seed1}")
            
            # Create second view
            torch.manual_seed(seed2)
            view2 = self.transform(image)
            print(f"Transform applied to view2 with seed={seed2}")
...
class LabeledDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        print(f"Setting seed for labeled augmentation: {seed} (idx={idx})")
        ...
        if self.transform:
            image = self.transform(image)
            print(f"Transform applied to labeled image: {img_name} (idx={idx})")
